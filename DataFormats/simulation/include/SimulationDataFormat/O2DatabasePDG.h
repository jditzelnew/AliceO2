...
//
// By using O2DatabasePDG::Instance() in our code instead of
...
class O2DatabasePDG
{
 ...
 public:
                       ...
                       TDatabasePDG* db = O2DatabasePDG::Instance())
  {
    ...
    // if we are here, try one last time to look for ground state of potential
    // isomere state
    pdg = pdg / 10 * 10;
    return MassImpl(db->GetParticle(pdg), success);
  }
...
*/
  ...
  // χc1(3872) aka X(3872), taken from PDG 2022
  // (https://pdg.lbl.gov/2022/listings/rpp2022-list-chi-c1-3872.pdf)
  db->AddParticle("Chi_c1(3872)", "Chi_c1(3872)", 3.87165, kFALSE, 0, 0,
                  "CCBarMeson", 9920443);
  ...
  //
  db->AddParticle("Upsilon(3S)", "Upsilon(3S)", 10.3552, kTRUE, 0, 0,
                  "Bottonium", 200553);

  // QCD diffractive states
  db->AddParticle("rho_diff0", "rho_diff0", 0, kTRUE, 0, 0, "QCD diffr. state",
                  9900110);
  db->AddParticle("pi_diffr+", "pi_diffr+", 0, kTRUE, 0, 3, "QCD diffr. state",
                  9900210);
  db->AddParticle("omega_di", "omega_di", 0, kTRUE, 0, 0, "QCD diffr. state",
                  9900220);
  db->AddParticle("phi_diff", "phi_diff", 0, kTRUE, 0, 0, "QCD diffr. state",
                  9900330);
  db->AddParticle("J/psi_di", "J/psi_di", 0, kTRUE, 0, 0, "QCD diffr. state",
                  9900440);
  db->AddParticle("n_diffr0", "n_diffr0", 0, kTRUE, 0, 0, "QCD diffr. state",
                  9902110);
  db->AddParticle("p_diffr+", "p_diffr+", 0, kTRUE, 0, 3, "QCD diffr. state",
                  9902210);
  ...
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Deuteron", "Deuteron", 1.87561294257, kTRUE, 0, 3, "Ion",
                    ionCode);
  }
  ...
  // Special particles
  //
  db->AddParticle("Cherenkov", "Cherenkov", 0, kFALSE, 0, 0, "Special",
                  kspe + 50);
  db->AddParticle("FeedbackPhoton", "FeedbackPhoton", 0, kFALSE, 0, 0,
                  "Special", kspe + 51);

  // Hyper nuclei and exotica
  ionCode = 1010010030;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("HyperTriton", "HyperTriton", 2.991134, kFALSE, 2.5e-15, 3,
                    "Ion", ionCode);
  ...
  ionCode = 1010010040;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Hyperhydrog4", "Hyperhydrog4", 3.922434, kFALSE, 2.5e-15,
                    3, "Ion", ionCode);
  ...
  ionCode = 1010020040;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Hyperhelium4", "Hyperhelium4", 3.921728, kFALSE, 2.5e-15,
                    6, "Ion", ionCode);
  ...
  ionCode = 1010020050;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Hyperhelium5", "Hyperhelium5", 4.839961, kFALSE, 2.5e-15,
                    6, "Ion", ionCode);
  ...
  ionCode = 1120020040;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("4XiHe", "4XiHe", 4.128, kFALSE, 4.04e-15, 3, "Ion",
                    ionCode);
  ...
  ionCode = 1000030040;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Lithium4", "Lithium4", 3.7513, kFALSE, 0.003, 9, "Ion",
                    ionCode);
  }
  // anti Lithium 4 ground state
  ionCode = -1000030040;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiLithium4", "AntiLithium4", 3.7513, kFALSE, 0.003, 9,
                    "Ion", ionCode);
  }
  ...
  ionCode = -1110020040;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiHyperhelium4sigma", "AntiHyperhelium4sigma", 3.995,
                    kFALSE, 2.5e-15, 6, "Ion", ionCode);
  }

  ionCode = 1010000020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("LambdaNeutron", "LambdaNeutron", 2.054, kFALSE, 2.5e-15, 0,
                    "Special", ionCode);
  }
  ...
  ionCode = 1020000020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Hdibaryon", "Hdibaryon", 2.23, kFALSE, 2.5e-15, 0,
                    "Special", ionCode);
  }

  ionCode = -1020000020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiHdibaryon", "AntiHdibaryon", 2.23, kFALSE, 2.5e-15, 0,
                    "Special", ionCode);
  }

  ionCode = 1010000030;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("LambdaNeutronNeutron", "LambdaNeutronNeutron", 2.99,
                    kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = -1010000030;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiLambdaNeutronNeutron", "AntiLambdaNeutronNeutron",
                    2.99, kFALSE, 2.5e-15, 0, "Special", ionCode);
  }

  ionCode = 1020010020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Xi0Proton", "Xi0Proton", 2.248, kFALSE, 5e-15, 3, "Ion",
                    ionCode);
  }

  ionCode = -1020010020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiXi0Proton", "AntiXi0Proton", 2.248, kFALSE, 5e-15, 3,
                    "Ion", ionCode);
  }

  ionCode = 1030000020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("OmegaProton", "OmegaProton", 2.592, kFALSE, 2.5e-15, 0,
                    "Special", ionCode);
  }
  ...
  ionCode = 1030010020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("OmegaNeutron", "OmegaNeutron", 2.472, kFALSE, 0.003, 3,
                    "Special", ionCode);
  }
  ...
  ionCode = 1060020020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("OmegaOmega", "OmegaOmega", 3.229, kFALSE, 2.5e-15, 6,
                    "Special", ionCode);
  }

  ionCode = -1060020020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiOmegaOmega", "AntiOmegaOmega", 3.229, kFALSE, 2.5e-15,
                    6, "Special", ionCode);
  }

  ionCode = 1010010021;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Lambda1405Proton", "Lambda1405Proton", 2.295, kFALSE, 0.05,
                    3, "Special", ionCode);
  }

  ionCode = -1010010021;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiLambda1405Proton", "AntiLambda1405Proton", 2.295,
                    kFALSE, 0.05, 3, "Special", ionCode);
  }

  ionCode = 1020000021;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Lambda1405Lambda1405", "Lambda1405Lambda1405", 2.693,
                    kFALSE, 0.05, 0, "Special", ionCode);
  }

  ionCode = -1020000021;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("AntiLambda1405Lambda1405", "AntiLambda1405Lambda1405",
                    2.693, kFALSE, 0.05, 0, "Special", ionCode);
  }

  ionCode = 2010010030;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("CTriton", "CTriton", 4.162, kFALSE, 3.3e-12, 3, "Ion",
                    ionCode);
  ...
  ionCode = 2010010020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("CDeuteron", "CDeuteron", 3.226, kFALSE, 3.3e-12, 3, "Ion",
                    ionCode);
  ...
  ionCode = 9010221;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f0_980", "f0_980", 0.980, kFALSE, 0.07, 0, "Resonance",
                    ionCode);
  }

  ionCode = 225;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f2_1270", "f2_1270", 1.275, kFALSE, 0.185, 0, "Resonance",
                    ionCode);
  }
  ...
  ionCode = 102134;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Lambda_1520_0", "Lambda_1520_0", 1.5195, kFALSE, 0.0156, 0,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("AntiLambda_1520_0", "AntiLambda_1520_0", 1.5195, kFALSE,
                    0.0156, 0, "Resonance", -ionCode);
  }

  // f1 study
  ionCode = 20223;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f1_1285", "f1_1285", 1.28210, kFALSE, 0.02420, 0,
                    "Resonance", ionCode);
  }
  ionCode = 20333;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f1_1420", "f1_1420", 1.42640, kFALSE, 0.05490, 0,
                    "Resonance", ionCode);
  }
  ...
  ionCode = 115;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("a2_1320", "a2_1320", 1.3182, kFALSE, 0.1078, 0,
                    "Resonance", ionCode);
  }
  ionCode = 10221;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f0_1370", "f0_1370", 1.37, kFALSE, 0.200, 0, "Resonance",
                    ionCode);
  }
  ionCode = 9030221;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f0_1500", "f0_1500", 1.500, kFALSE, 0.112, 0, "Resonance",
                    ionCode);
  }
  ionCode = 10331;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f0_1710", "f0_1710", 1.710, kFALSE, 0.139, 0, "Resonance",
                    ionCode);
  }
  ionCode = 335;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("f2_1525", "f2_1525", 1.525, kFALSE, 0.073, 0, "Resonance",
                    ionCode);
  }
  ...
  ionCode = 123314;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Xi_Minus_1820", "Xi_Minus_1820", 1.8234, kFALSE, 0.024, -3,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Xi_Plus_1820", "Xi_Plus_1820", 1.8234, kFALSE, 0.024, 3,
                    "Resonance", -ionCode);
  }
  ...
  ionCode = 123324;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Xi_0_1820", "Xi_0_1820", 1.8234, kFALSE, 0.024, 0,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Xi_0_Bar_1820", "Xi_0_Bar_1820", 1.8234, kFALSE, 0.024, 0,
                    "Resonance", -ionCode);
  }
  ...
  ionCode = 9322134;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("ps_2100", "ps_2100", 2.100, kFALSE, 0.040, 3, "Resonance",
                    ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("anti-ps_2100", "anti-ps_2100", 2.100, kFALSE, 0.040, -3,
                    "Resonance", -ionCode);
  }

  ionCode = 9322136;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("ps_2500", "ps_2500", 2.500, kFALSE, 0.040, 3, "Resonance",
                    ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("anti-ps_2500", "anti-ps_2500", 2.500, kFALSE, 0.040, -3,
                    "Resonance", -ionCode);
  }
  ...
  ionCode = 9322132;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_Plus_1870", "Ps_Plus_1870", 1.870, kFALSE, 0.10, 3,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
  ...
  ionCode = 9322312;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_Plus_2065", "Ps_Plus_2065", 2.065, kFALSE, 0.10, 3,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
  ...
  ionCode = 9323212;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_Plus_2255", "Ps_Plus_2255", 2.255, kFALSE, 0.10, 3,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
  ...
  ionCode = 9332212;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_Plus_2455", "Ps_Plus_2455", 2.455, kFALSE, 0.10, 3,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
  ...
  ionCode = 9322131;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_0_1870", "Ps_0_1870", 1.870, kFALSE, 0.10, 0,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Anti-Ps_0_1870", "Anti-Ps_0_1870", 1.870, kFALSE, 0.10, 0,
                    "Resonance", -ionCode);
  }
  ionCode = 9322311;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_0_2065", "Ps_0_2065", 2.065, kFALSE, 0.10, 0,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Anti-Ps_0_2065", "Anti-Ps_0_2065", 2.065, kFALSE, 0.10, 0,
                    "Resonance", -ionCode);
  }
  ionCode = 9323211;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_0_2255", "Ps_0_2255", 2.255, kFALSE, 0.10, 0,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Anti-Ps_0_2255", "Anti-Ps_0_2255", 2.255, kFALSE, 0.10, 0,
                    "Resonance", -ionCode);
  }
  ionCode = 9332211;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Ps_0_2455", "Ps_0_2455", 2.455, kFALSE, 0.10, 0,
                    "Resonance", ionCode);
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Anti-Ps_0_2455", "Anti-Ps_0_2455", 2.455, kFALSE, 0.10, 0,
                    "Resonance", -ionCode);
  }
  ...
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Anti-Theta_c_3100", "Anti-Theta_c_3100", 3.099, kFALSE,
                    83.e-6, 0, "Resonance",
                    ionCode); // same width as D*+ (83 keV)
  }
  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("Theta_c_3100", "Theta_c_3100", 3.099, kFALSE, 83.e-6, 0,
                    "Resonance", -ionCode); // same width as D*+ (83 keV)
  }

  // d*(2380) - dibaryon resonance

  ionCode = 900010020; // Arbitrary choice - as deuteron but with leading 9
                       // instead of 10
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("d*_2380", "d*_2380", 2.38, kFALSE, 0.070, 3, "Resonance",
                    ionCode);
  }
  ...
  ionCode = 900000020;
  if (!db->GetParticle(ionCode)) {
    db->AddParticle("Sexaquark", "Sexaquark", 2.0, kTRUE, 0.0, 0, "Special",
                    ionCode);
  }

  if (!db->GetParticle(-ionCode)) {
    db->AddParticle("AntiSexaquark", "AntiSexaquark", 2.0, kTRUE, 0.0, 0,
                    "Special", -ionCode);
  }
